...
(() => {
  ...
  function updateBookingMessage() {
    const content = window.BEAUTY_STUDIO_CONTENT || {};
    const rules = content.bookingRules || {};
    const lang = localStorage.getItem("beauty_studio_language") || "en";
    const pick = (obj, base, fallback = "") => {
      if (!obj) return fallback;
      const key = lang === "zh" ? `${base}Zh` : lang === "my" ? `${base}My` : base;
      return String(obj[key] ?? obj[base] ?? fallback ?? "");
    };
    const message = pick(rules, "bookingMessage", pick(content, "bookingMessage", "")).trim();
    if (!message) return;
  ...
  window.addEventListener("beautyStudioLanguageChanged", () => {
    ...
    // Re-render the currently visible time grid so generated labels follow the active language.
    try {
      const active = modal.querySelector('.date-choice.active, .date-choice.selected');
      if (active && active.dataset?.date) renderTimes(new Date(active.dataset.date));
    } catch (_) {}
  });
...
(function(){
  ...
  function addToggle(){
    let wrap=document.getElementById('beautyLangWrap');
    if(!wrap){
      wrap=document.createElement('div');
      wrap.className='beauty-lang-wrap';
      wrap.id='beautyLangWrap';
      wrap.innerHTML=`
    ...
    document.addEventListener('click',e=>{
      if(!wrap.contains(e.target)){
        menu.classList.remove('open'); b.classList.remove('open'); b.setAttribute('aria-expanded','false');
      }
    });
  }
